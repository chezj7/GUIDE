...
class Agent:
    ...
    def update_planning_state(self, robot_locations):
        ...
        self.node_coords, self.utility, self.guidepost, self.occupancy, self.adjacent_matrix, self.current_index, self.neighbor_indices, self.is_unknown_node = \
            self.node_manager.get_all_node_graph(self.map_info,self.location, robot_locations,self.regions_state,self.unknown_centers)

    def get_observation(self):
        node_coords = self.node_coords
        raw_node_coords = node_coords.copy()
        node_utility = self.utility.reshape(-1, 1)
        node_guidepost = self.guidepost.reshape(-1, 1)
        node_occupancy = self.occupancy.reshape(-1, 1)
        self.raw_node_coords = raw_node_coords
        node_is_unknown = self.is_unknown_node.reshape(-1, 1)
        # node_value = self.get_node_value_vector()
        ...
        node_inputs = np.concatenate((node_coords, node_utility, node_guidepost, node_occupancy,node_is_unknown), axis=1)
    ...
    def get_node_value_vector(self):
        if not hasattr(self, 'global_path') or self.global_path is None:
            print("[Debug] global_path is None or not set!")
            return np.zeros((self.node_coords.shape[0], 1))

        if not hasattr(self, 'raw_node_coords'):
            print("[Error] raw_node_coords not found!")
            return np.zeros((self.node_coords.shape[0], 1))

        node_coords_list = [tuple(np.round(coord, 3)) for coord in self.raw_node_coords.tolist()]
        value_vector = np.zeros((len(node_coords_list), 1))

        print(f"[Debug] global_path length: {len(self.global_path)}")
        print(f"[Debug] First 5 path entries: {self.global_path[:5]}")

        matched = 0
        for i, p in enumerate(self.global_path):
            p_tuple = tuple(np.round(p, 3))
            if p_tuple in node_coords_list:
                idx = node_coords_list.index(p_tuple)
                value_vector[idx] = 1.0 - i / len(self.global_path)
                matched += 1
            # else:
                # print(f"[Warn] path point {p_tuple} not matched in node_coords")

        print(f"[Debug] Matched {matched} nodes from global_path to node_coords.")
        return value_vector
